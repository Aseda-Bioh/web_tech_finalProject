    ...
    try {
        ...
        if ($result->num_rows > 0) {
            ...
            $activityStmt = $conn->prepare("
                ...
                VALUES (?, 'Quiz_Completed', ?, 'Started RPG quiz')
            ");
...
function endQuiz($conn) {
    ...
    try {
        ...
        // Check for RPG Sage achievement
        if ($stats['total_score'] >= 400) { // Example threshold
            $updateAchievement = $conn->prepare("
            UPDATE user_achievements ua
            JOIN achievements a ON ua.achievement_id = a.achievement_id
            SET ua.progress = ua.progress + 1,
                ua.progress_percentage = (ua.progress * 100.0) / a.requirement_value
            WHERE ua.user_id = ? AND ua.achievement_id = 4
        ");
        $updateAchievement->bind_param('i', $userId);
